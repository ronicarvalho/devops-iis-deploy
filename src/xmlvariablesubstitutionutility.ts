...
export function substituteAppSettingsVariables(folderPath, isFolderBasedDeployment, fileName?: string) {
    ...
    let parameterFilePath = !fileName || String(fileName).toLocaleLowerCase().indexOf("parameters.xml") != -1 ? path.join(folderPath, 'parameters.xml') : null;
    ...
    let tags = ["applicationSettings", "appSettings", "connectionStrings", "configSections", "system.serviceModel"];
        ...
        for (let xmlNode of nodes) {
            if (varUtility.isObject(xmlNode)) {
                ...
                try {
                    ...
                    else if (xmlNode.nodeName == "system.serviceModel") {
                        var client = npmDomUtiltiyInstance.getChildElementsByTagName(xmlNode, "client");
                        isSubstitutionApplied = updateXmlEndpointNodeAttribute(xmlNode, variableMap, replacableTokenValues) || isSubstitutionApplied;
                    }
                    else {
...
function updateXmlEndpointNodeAttribute(xmlDomNode, variableMap, replacableTokenValues): boolean {

    let isSubstitutionApplied: boolean = false;
    const ConfigFileEndpointToken = 'CONFIG_FILE_ENDPOINT_TOKEN';
    ...
    if (xmlDomNode.attributes) {
        if (xmlDomNode.hasAttribute("address")) {
            var endpointName = xmlDomNode.getAttribute("name");
            if (endpointName && variableMap[endpointName]) {
                let ConfigFileEndpointTokenName = ConfigFileEndpointToken + '(' + endpointName + ')';
                tl.debug(tl.loc('SubstitutingEndpointValue', endpointName, ConfigFileEndpointTokenName));
                xmlDomNode.setAttribute("address", ConfigFileEndpointTokenName);
                replacableTokenValues[ConfigFileEndpointTokenName] = variableMap[endpointName].replace(/"/g, "'");
                isSubstitutionApplied = true;
            }
            else if (variableMap["address"] != undefined) {
                let ConfigFileEndpointTokenName = ConfigFileEndpointToken + '(address)';
                tl.debug(tl.loc('SubstitutingEndpointValue', endpointName, ConfigFileEndpointTokenName));
                xmlDomNode.setAttribute("address", ConfigFileEndpointTokenName);
                replacableTokenValues[ConfigFileEndpointTokenName] = variableMap["address"].replace(/"/g, "'");
                isSubstitutionApplied = true
            }
        }
    }
    ...
    if (xmlDomNode.childNodes) {
        ...
        for (let childs = 0; childs < children.length; childs++) {
            ...
            if (varUtility.isObject(childNode)) {
                isSubstitutionApplied = updateXmlEndpointNodeAttribute(childNode, variableMap, replacableTokenValues) || isSubstitutionApplied;
            }
        }
    }
    return isSubstitutionApplied;
